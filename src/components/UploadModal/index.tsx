  ...
  return (
    <Modal
      ...
      onOk={() => {
        ...
        const texts: string[] = [];
        fileContent?.map((item: any) => {
          texts.push(...(fileMap.get(item.name) || []));
        })
        setUploadLoading(true);
        axios.post('http://127.0.0.1:5000/store_text_embeddings', {
          texts,
    ...
    >
      <Form getFormApi={formApi => formRef.current = formApi}>
        <Form.Select
          ...
          rules={[
            { required: true, message: '请选择或者填写新建名称知识库' },
          ]}
        >
        ...
        <Form.Upload
          ...
          customRequest={({ file, onProgress }) => {
            ...
            reader.onload = (e) => {
              if (file.fileInstance?.name) {
                const contentArr = splitTextWithOverlap(e.target?.result as string, 40, 10);
          ...
          accept="text/plain"
          rules={[
            { required: true, message: '请上传知识库文件' },
          ]}
        >
